...
reserveTestTable.add(['出張ビジネスプラン','Sunday','1','1','on','on','off','上杉景虎','希望しない','','','off','シングル','11375','横向き']);
...
Feature('Guest予約機能_By_Nexus9');

Data(reserveTestTable).Scenario('Guest予約_部屋タイプ確認_料金確認_By_Nexus9', async({I , current}) => {
...
//Guestユーザーで予約
    ...
    if((current.宿泊プラン == 'お得な特典付きプラン') || (current.宿泊プラン == 'プレミアムプラン') || (current.宿泊プラン == '素泊まり') || (current.宿泊プラン == '出張ビジネスプラン') || (current.宿泊プラン == 'カップル限定プラン')){
        ...
        I.see(current.部屋タイプ, {css: 'body > div > div:nth-child(2) > div > p:nth-child(2)'});
        I.switchTo();
    }else{
        I.see(current.部屋タイプ);
    }
